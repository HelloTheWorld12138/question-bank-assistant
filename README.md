...
和 WMF/EMF 题图会在本机自动转换为 PNG 并保留原貌，不要求教师安装额外工具；需要
继续编辑的公式仍可人工改写为 LaTeX。上传图片可以指定放入题目、答案或解析，也可以在入库前和后续
编辑时移除；常见位图统一保存为 PNG，试卷导出时按正文宽度稳定排版。公式预览组件
和旧图转换组件均随项目本地提供，不依赖 CDN 或境外网络。Word 中的斜体、粗体和
